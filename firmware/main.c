...
static void btn_task_sem(void *arg)
{
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
             ...
             */
            ESP_LOGI(TAG, "[sem] Emergency Stop Button Pressed #%lu  latency=%lld us (max=%llu)",
...
static void btn_task_notif(void *arg)
{
    for (;;) {
        ...
        ESP_LOGI(TAG, "[notif] Ride Control Alert #%lu  latency=%lld us (max=%llu) "
                      "(notif count=%lu)",
                 (unsigned long)presses_observed,
                 (long long)lat,
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "==== App 3 [Theme Park Dispatcher] starting — ISR + bottom-half ====");
    ...
    xTaskCreatePinnedToCore(btn_task_sem,  "btn_sem",   2048, NULL, 12, NULL, APP_CPU_NUM);
    xTaskCreatePinnedToCore(btn_task_notif,"btn_notif", 2048, NULL, 12,
